...
static int16_t saadc_read_ain7(void)
{
    ...
    SAADC_CH0_CONFIG = SAADC_CONFIG_GAIN_1_6 | SAADC_CONFIG_REFSEL_INT | SAADC_CONFIG_TACQ_40US | SAADC_CONFIG_MODE_SE;
...
static uint8_t mv_to_percent(uint16_t mv)
{
    if (mv >= 4200)
        return 100;
    if (mv >= 4100)
        return 90 + (uint32_t)(mv - 4100) * 10 / 100;
    if (mv >= 3950)
        return 75 + (uint32_t)(mv - 3950) * 15 / 150;
    if (mv >= 3800)
        return 60 + (uint32_t)(mv - 3800) * 15 / 150;
    if (mv >= 3600)
        return 20 + (uint32_t)(mv - 3600) * 40 / 200;
    if (mv >= 3000)
        return (uint32_t)(mv - 3000) * 20 / 600;
    return 0;
}
